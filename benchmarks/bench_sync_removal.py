"""Benchmark sync removal impact on hash table build + kernel map pipeline."""
...
def main():
    ...
    print(
        f"{'N':>10s}  {'HT Build':>10s}  {'Search':>10s}  {'Count':>10s}  {'Cumsum':>10s}  {'Scatter':>10s}  {'Pipeline':>10s}"
    )
    print("-" * 84)
    ...
    for num_coords in [10_000, 50_000, 100_000, 200_000, 500_000, 1_000_000]:
        ...
        # Benchmark hash table build (includes prepare + insert, no explicit sync)
        def build_ht():
            return TorchHashTable.from_keys(coords, hash_method=HashMethod.CITY, device=device)

        t_build, _ = bench_cuda_events(build_ht)
        ...
        print(
            f"{num_coords:>10,}  {t_build:>9.3f}ms  {t_search:>9.3f}ms  {t_count:>9.3f}ms  {t_cumsum:>9.3f}ms  {t_scatter:>9.3f}ms  {t_pipe:>9.3f}ms"
        )
