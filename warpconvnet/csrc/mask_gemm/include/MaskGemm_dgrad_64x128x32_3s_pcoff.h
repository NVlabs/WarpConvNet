...
struct MaskGemm_dgrad_64x128x32_3s_pcoff {
...
#undef N_out
    ...
    while (_mw_has_next()) {
      ...
      if (num_k_tiles == 1) {
        ...
        _load_B_tile(ptr_Bk, sB(_, _, 0), gmem_thr_copy_B, n_start, 0, C_in, C_out, n_full_tile);
        cute::cp_async_fence();
      ...
      } else {
        {
          ...
          for (int s = 0; s < NumStages - 1; ++s) {
            if (s < prolog_tiles) {
              _load_A_with_offsets(ptr_A, a_state, sA(_, _, s), s * tK, C_out, stride_A);
              _load_B_tile(
                  ptr_Bk, sB(_, _, s), gmem_thr_copy_B, n_start, s * tK, C_in, C_out, n_full_tile);
            }
            cute::cp_async_fence();
          }
        }
            ...
            switch (kb) {
              case 0:
                _load_A_kblock<decltype(sA(_, _, 0)), 0>(
                    ptr_A, a_state, sA(_, _, smem_pipe_write), k_start, C_out);
                _load_B_kblock<decltype(sB(_, _, 0)), 0>(
                    ptr_Bk, sB(_, _, smem_pipe_write), n_start, k_start, C_in, C_out);
                break;
              case 1:
                if (K_BLOCK_MAX_STATIC > 1)
                  _load_A_kblock<decltype(sA(_, _, 0)), 1>(
                      ptr_A, a_state, sA(_, _, smem_pipe_write), k_start, C_out);
                _load_B_kblock<decltype(sB(_, _, 0)), 1>(
                    ptr_Bk, sB(_, _, smem_pipe_write), n_start, k_start, C_in, C_out);
                break;
              case 2:
                if (K_BLOCK_MAX_STATIC > 2)
                  _load_A_kblock<decltype(sA(_, _, 0)), 2>(
                      ptr_A, a_state, sA(_, _, smem_pipe_write), k_start, C_out);
                if (K_BLOCK_MAX_STATIC > 2)
                  _load_B_kblock<decltype(sB(_, _, 0)), 2>(
                      ptr_Bk, sB(_, _, smem_pipe_write), n_start, k_start, C_in, C_out);
                break;
              case 3:
                if (K_BLOCK_MAX_STATIC > 3)
                  _load_A_kblock<decltype(sA(_, _, 0)), 3>(
                      ptr_A, a_state, sA(_, _, smem_pipe_write), k_start, C_out);
                if (K_BLOCK_MAX_STATIC > 3)
                  _load_B_kblock<decltype(sB(_, _, 0)), 3>(
                      ptr_Bk, sB(_, _, smem_pipe_write), n_start, k_start, C_in, C_out);
                break;
            }
            if (kb % 2 == 0)
        ...
        // MMA reads smem before its cp.async completes — same race
        // class as the forward 64x128x32_3s K=8 fp16 silent-NaN.
        cute::cp_async_wait<0>();
        __syncthreads();
...
private:
#include "warpgemm_dgrad_helpers_iter.cuh"
};
}  // namespace cute_gemm
}  // namespace warpconvnet
