...
// Config: MaskGemm_forward_64x64x32_1s_flat
...
struct MaskGemm_forward_64x64x32_1s_flat {
  ...
  __device__ void operator()(const ElementInput *ptr_A,
    ...
    while (offsets_remaining) {
      ...
      if (num_k_tiles == 1) {
        ...
        // No __syncthreads here — next offset uses the OTHER stage (if NumStages>1)
        ...
        for (int kt = 0; kt < num_k_tiles; ++kt) {
          ...
          int smem_stage = (NumStages > 1) ? (kt & 1) : 0;
...
private:
  ...
  __device__ void _load_B_tile(const ElementInput *ptr_Bk,
    ...
    // CuTe TiledCopy only works for 2-byte elements (fp16/bf16).
    // For fp32 (4-byte), the GmemTiledCopyB layout is incompatible — use cpasync fallback.
    if (n_full_tile && k_full_tile && sizeof(ElementInput) <= 2) {
      // Full tile: use CuTe TiledCopy (correct and fast, fp16/bf16 only)
