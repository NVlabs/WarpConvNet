...
struct MaskGemm_forward_64x64x32_2s_pipelined {
  ...
  __device__ void operator()(const ElementInput *ptr_A,
    ...
    if (!offsets_remaining) goto epilogue;

    if (num_k_tiles == 1) {
      // --- Single k-tile: pipeline MMA(N) with cp.async(N+1) across offsets ---
      int smem_stage = 0;

      // Prolog: load first offset
      int k_cur = __ffs(offsets_remaining) - 1;
      ...
      while (true) {
        if (offsets_remaining) {
          int k_next = __ffs(offsets_remaining) - 1;
          offsets_remaining &= offsets_remaining - 1;
          ...
          _update_A_indices(a_state,
                            ...
                            k_next,
                            K);
          _load_A_with_offsets(ptr_A, a_state, sA(_, _, write_stage), 0, C_in);
      ...
      while (offsets_remaining) {
        ...
        for (int kt = 0; kt < num_k_tiles; ++kt) {
          ...
          _load_A_with_offsets(ptr_A, a_state, sA(_, _, 0), k_start_cin, C_in);
          _load_B_tile(
