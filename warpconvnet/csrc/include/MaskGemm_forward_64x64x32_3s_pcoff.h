...
struct MaskGemm_forward_64x64x32_3s_pcoff {
        ...
        if (num_k_tiles == 1) {
          ...
          // ---- Multi k-tile: pipelined with interleaved A loads ----
          {
            ...
            for (int s = 0; s < NumStages - 1; ++s) {
              if (s < prolog_tiles) {
                _load_A_with_offsets(ptr_A, a_state, sA(_, _, s), s * tK, C_in, stride_A);
                _load_B_tile(ptr_Bk,
                             sB(_, _, s),
                             gmem_thr_copy_B,
                             n_start,
              ...
              switch (kb) {
                case 0:
                  _load_A_kblock<decltype(sA(_, _, 0)), 0>(
                      ptr_A, a_state, sA(_, _, smem_pipe_write), k_start_cin, C_in, stride_A);
                  _load_B_tile(ptr_Bk,
