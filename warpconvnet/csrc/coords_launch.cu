...
extern "C" __global__ void search_kernel_fnv1a(
    const int* table_kvs, const int* vector_keys, const int* search_keys,
    int* results, int num_search_keys, int key_dim, int table_capacity);
extern "C" __global__ void search_kernel_city(
    const int* table_kvs, const int* vector_keys, const int* search_keys,
    int* results, int num_search_keys, int key_dim, int table_capacity);
extern "C" __global__ void search_kernel_murmur(
    const int* table_kvs, const int* vector_keys, const int* search_keys,
    int* results, int num_search_keys, int key_dim, int table_capacity);

extern "C" __global__ void expand_insert_kernel_fnv1a(
    int* table_kvs, int* vector_keys, const int* base_coords, const int* offsets,
    int num_base_coords, int num_offsets, int key_dim, int table_capacity,
    int vector_capacity, int* num_entries_ptr, int* status_ptr);
extern "C" __global__ void expand_insert_kernel_city(
    int* table_kvs, int* vector_keys, const int* base_coords, const int* offsets,
    int num_base_coords, int num_offsets, int key_dim, int table_capacity,
    int vector_capacity, int* num_entries_ptr, int* status_ptr);
extern "C" __global__ void expand_insert_kernel_murmur(
    int* table_kvs, int* vector_keys, const int* base_coords, const int* offsets,
    int num_base_coords, int num_offsets, int key_dim, int table_capacity,
    int vector_capacity, int* num_entries_ptr, int* status_ptr);
...
extern "C" __global__ void assign_order_discrete_16bit_kernel(
    const int* bcoords_data, int num_points, int64_t* result_order);
extern "C" __global__ void assign_order_discrete_20bit_kernel_4points(
    const int* coords_data, int num_points, int64_t* result_order);
...
__device__ static int64_t morton_code_20bit_device_ctc(int64_t coord_x, int64_t coord_y, int64_t coord_z) {
    return (part1by2_long_ctc(coord_z) << 2) | (part1by2_long_ctc(coord_y) << 1) | part1by2_long_ctc(coord_x);
}

__global__ void coord_to_code_kernel_impl(
...
void coords_hashmap_prepare(torch::Tensor table_kvs, int capacity) {
    ...
    prepare_key_value_pairs_kernel<<<blocks, threads, 0, stream>>>(
        table_kvs.data_ptr<int>(), capacity);
}
...
void coords_hashmap_insert(torch::Tensor table_kvs, torch::Tensor vector_keys,
    ...
    switch (hash_method) {
        case 0: insert_kernel_fnv1a<<<blocks, threads, 0, stream>>>(tbl, vk, num_keys, key_dim, capacity); break;
        case 1: insert_kernel_city<<<blocks, threads, 0, stream>>>(tbl, vk, num_keys, key_dim, capacity); break;
        case 2: insert_kernel_murmur<<<blocks, threads, 0, stream>>>(tbl, vk, num_keys, key_dim, capacity); break;
...
void coords_hashmap_search(torch::Tensor table_kvs, torch::Tensor vector_keys,
    ...
    switch (hash_method) {
        case 0: search_kernel_fnv1a<<<blocks, threads, 0, stream>>>(tbl, vk, sk, res, num_search, key_dim, capacity); break;
        case 1: search_kernel_city<<<blocks, threads, 0, stream>>>(tbl, vk, sk, res, num_search, key_dim, capacity); break;
        case 2: search_kernel_murmur<<<blocks, threads, 0, stream>>>(tbl, vk, sk, res, num_search, key_dim, capacity); break;
        default: TORCH_CHECK(false, "Invalid hash_method: ", hash_method);
    }
}
...
void coords_hashmap_expand(torch::Tensor table_kvs, torch::Tensor vector_keys,
    ...
    switch (hash_method) {
        case 0: expand_insert_kernel_fnv1a<<<blocks, threads, shared_mem, stream>>>(
            tbl, vk, bc, off, num_base, num_offsets, key_dim, capacity, vector_capacity, ne, st); break;
        case 1: expand_insert_kernel_city<<<blocks, threads, shared_mem, stream>>>(
            tbl, vk, bc, off, num_base, num_offsets, key_dim, capacity, vector_capacity, ne, st); break;
        case 2: expand_insert_kernel_murmur<<<blocks, threads, shared_mem, stream>>>(
            tbl, vk, bc, off, num_base, num_offsets, key_dim, capacity, vector_capacity, ne, st); break;
        default: TORCH_CHECK(false, "Invalid hash_method: ", hash_method);
    }
}
...
void coords_kernel_map_offset(torch::Tensor table_kvs, torch::Tensor vector_keys,
    ...
    dim3 grid((num_query + threads_x - 1) / threads_x,
              (num_offsets + threads_y - 1) / threads_y);
    ...
    switch (hash_method) {
        case 0: kernel_map_offset_fnv1a<<<grid, block, 0, stream>>>(
            tbl, vk, qc, ko, out, num_query, key_dim, num_offsets, capacity); break;
        case 1: kernel_map_offset_city<<<grid, block, 0, stream>>>(
            tbl, vk, qc, ko, out, num_query, key_dim, num_offsets, capacity); break;
        case 2: kernel_map_offset_murmur<<<grid, block, 0, stream>>>(
            tbl, vk, qc, ko, out, num_query, key_dim, num_offsets, capacity); break;
        default: TORCH_CHECK(false, "Invalid hash_method: ", hash_method);
    }
}
...
void coords_map_found_indices_to_maps(torch::Tensor found, torch::Tensor mapped,
    ...
    map_found_indices_to_maps_cuda<<<blocks, threads, 0, stream>>>(
        found.data_ptr<int>(), mapped.data_ptr<int>(), offsets.data_ptr<int>(),
        in_maps.data_ptr<int>(), out_maps.data_ptr<int>(), K, M);
}
...
void coords_kernel_map_size_4d(torch::Tensor table_kvs, torch::Tensor vector_keys,
    ...
    dim3 block(threads_x, threads_y);
    dim3 grid((num_query + threads_x - 1) / threads_x,
              (num_kernels + threads_y - 1) / threads_y);
    ...
    switch (hash_method) {
        case 0: kernel_map_size_4d_fnv1a<<<grid, block, 0, stream>>>(
            tbl, vk, qc, ks, out, num_query, capacity, num_kernels); break;
        case 1: kernel_map_size_4d_city<<<grid, block, 0, stream>>>(
            tbl, vk, qc, ks, out, num_query, capacity, num_kernels); break;
        case 2: kernel_map_size_4d_murmur<<<grid, block, 0, stream>>>(
            tbl, vk, qc, ks, out, num_query, capacity, num_kernels); break;
        default: TORCH_CHECK(false, "Invalid hash_method: ", hash_method);
    }
}
...
void coords_find_first_gt_bsearch(torch::Tensor offsets_tensor, int M,
    ...
    find_first_gt_bsearch<<<blocks, threads, shared_mem, stream>>>(
        offsets_tensor.data_ptr<int>(), M, indices.data_ptr<int>(), N,
        output.data_ptr<int>());
}
...
void coords_coord_to_code(torch::Tensor grid_coord, torch::Tensor coord_offset,
    ...
    int threads = 256;
    int blocks = (N + threads - 1) / threads;
    coord_to_code_kernel_impl<<<blocks, threads, 0, stream>>>(
