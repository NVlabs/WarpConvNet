...
Two layers:
  ...
  - AUTHORED FIELD (runs once warpgemm emits per-tile ``dispatch_mask_words``,
    dev-log): the literals must equal {tile : 2 not in dispatch_mask_words}.
...
def test_mask_words_tiers_match_snapshot_union():
    ...
    if not tiles or not hasattr(tiles[0], "dispatch_mask_words"):
        pytest.skip("dispatch_mask_words not present yet (dev-log pending)")
    union = set()
    for t in tiles:
...
def test_mw1_authorized_tiles_are_guarded(id_lo, id_hi, authored):
    ...
    (dev-log): every tile AUTHORIZED only at MW1 (2 not in dispatch_mask_words)
    ...
    if sample is None or not hasattr(sample, "dispatch_mask_words"):
        pytest.skip(
            "warpgemm tile_metadata has no dispatch_mask_words yet (dev-log "
