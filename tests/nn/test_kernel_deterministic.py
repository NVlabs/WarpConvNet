...
class TestLargeN:
    ...
    def test_mask_gemm_fp16(self, large_problem):
        d = large_problem
        # Tile 41 was wrong for C_in <= 32 on Blackwell (single-k-tile
        # mainloop race, fixed at the warpgemm generator level); this test
        # deliberately pins the tile and now runs at every C_in as a
        # regression check on that fix.
        out = _execute_forward(
            algo="mask_gemm",
            params={"tile_id": 41},
            in_features=d["in_feats_32"].half(),
